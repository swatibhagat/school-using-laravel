			...
			<div class="">				 
					<div class="box-body">
						<table id="memTable" class="table table-bordered table-striped">
							...
							<tr>								
								<td>{{$i++}}</td>
								<td><?php //print_r($course);?>
								@if($course!="")
									@foreach ($course as $co)
										@if($co->course_id == $batch->course_id)
											{{$co->name}}
										@endif
									@endforeach
								@endif</td>							
								...
								<td><a class="icon-pencil" title="" href="{{url("/batch/edit/$batch->batch_id")}}"><i class="fa fa-pencil" aria-hidden="true"></i></a> 
								 
								<a class="icon-cross2" onclick="return confirmDel()" title="" href="{{url("/batch/delete/$batch->batch_id")}}"><i class="fa fa-trash" aria-hidden="true"></i></a></td>
							</tr>  
							@endforeach	
							@endif						
							</tbody> 

...
function showUser(str) {
  ...
  if (str === "" && a_date ==="") {
    ...
    return;
	  ...
	  xmlhttp.open("GET","emplyoyee_daily_attendance/select/"+str,true);
	  xmlhttp.send();
	}
  
}
/*Attendence enter in db*/

function taking_attendabce(emp_code, str){

	var a_date=document.getElementById("a_date").value;
	var dept=document.getElementById("dept").value;
	var data = a_date+"---"+dept+"---"+emp_code+"---"+str;
	if(a_date!=" " && dept!=" " && emp_code!=" " && str!=" ")
	{
		if (window.XMLHttpRequest) {
		// code for IE7+, Firefox, Chrome, Opera, Safari
		xmlhttp=new XMLHttpRequest();
	  ...
	  xmlhttp.open("GET","taking_attendance/take/"+data,true);
	  xmlhttp.send();
	}

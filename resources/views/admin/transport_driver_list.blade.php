			...
			<div class="">				 
					<div class="box-body">
						<table id="memTable" class="table table-bordered table-striped">
							...
							@foreach ($data as $diver)
							<tr>								
								<td>{{$i++}}</td>
								<td>@if($data1!="")
							@foreach($data1 as $dat)
						@if($diver->vehicle_no == $dat->v_id) {{$dat->vehicle_no}} @endif
							@endforeach
						@endif
								
								</td>
								<td>{{$diver->drive_name}}</td>
								<td>{{$diver->license_no}}</td>
								
								<td><a class="icon-pencil" title="" href="{{url("/driver/edit/$diver->d_id")}}"><i class="fa fa-pencil" aria-hidden="true"></i></a> 
								 
								<a class="icon-cross2" onclick="return confirmDel()" title="" href="{{url("/driver/delete/$diver->d_id")}}"><i class="fa fa-trash" aria-hidden="true"></i></a></td>
							</tr>  
							@endforeach	
							@endif						
							</tbody> 
